...
// Confirms the request carries a valid Supabase session belonging to an admin.
// Returns the user object if allowed, or null if not.
...
export default async function handler(req, res) {
  ...
  // ── Create a new coupon + promotion code ───────────────────
  if (req.method === 'POST') {
    try {
      const {
        code,
  ...
  // Stripe promotion codes can't be deleted, only turned on/off.
  if (req.method === 'PATCH') {
    try {
