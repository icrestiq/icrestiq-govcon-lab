...
// The `with { type: 'json' }` import attribute is required for this to run
// under Vercel's serverless functions (native Node ESM, which refuses a
// bare JSON import without it) — Vite's own JSON handling ignores the
// attribute and keeps working exactly as before for the frontend build.
import naicsData from '../data/naics.json' with { type: 'json' }
