...
- **Latest commit:** `7251f32` — "Deal identifiers, remove-linked-company, and two visual fixes" — pushed 2026-08-24. Preceded the same day by `46de002` (CRM Phase 2), `c37d2ca` (handoff), `4c782fe` (CRM Phase 1), `2405d1a` (handoff), `0ff27e7` (CRM Phase 0), `744199b` (webhook price-map fix), and `a499b79` (Founding pricing).
- **Uncommitted working-tree state:** only `.claude/`, untracked, not touched this session. Working tree is otherwise clean — everything through Phase 2 is committed and pushed. The one change *not* represented in git is a same-day Supabase Edge Function deploy (`generate_suggested_bid` v14, an NSN-extraction bug fix) plus a one-row production data backfill — see the Session entry below; Edge Functions are never checked into this repo (see Current Architecture).
...
- **Native CRM ("Sourcing Pipeline")** — Phases 0–2 shipped and live-verified 2026-08-24 (see Implemented in Current Code and the Session entries below). Remaining: **Quotes into Proposal Builder** (explicitly deferred out of Phase 2 as its own follow-up, not yet started), Phase 3 (activity/tasks + the note-sharing/flagging/moderation UI), Phase 4 (reporting; **search suppliers/packers-shippers by CAGE code, name, and NSN — specifically requested 2026-08-24**, needs schema work first since `companies` has no `cage_code` field and NSNs aren't linked to which supplier can provide them), Phase 5 (HubSpot-parity stretch + adding Keith's other business entities to the shared directory). Scoping artifact: "Sourcing Pipeline" (kept in sync with the actual build this session, not linked in git).
...
**Next recommended action:** Decide whether to audit other historical `bid_requests` rows for the same NSN-extraction gap, given it's a real (if minor) data-completeness issue on already-paid purchases — echoes the still-unresolved `match_opportunities` orphaned-purchase audit from 2026-08-23 in shape, if not severity. This is a recommendation, not an approved priority.
